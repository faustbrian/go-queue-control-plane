...
func TestPostgresRetentionJobResultIntegration(t *testing.T) {
	...
	if _, err := runtime.Commands.Get(
		ctx, "tenant-command-retention", "recent-retry",
	); !errors.Is(err, controlpostgres.ErrCommandNotFound) {
...
func migrateIntegrationDatabase(ctx context.Context, dsn string) error {
	...
	if len(first.Records()) != 8 {
		return errors.New("integration: complete migration history was not applied once")
	}
