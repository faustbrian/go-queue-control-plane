# go-queue-control-plane

`go-queue-control-plane` is the administrative control plane for
[`go-queue`](https://github.com/faustbrian/go-queue). It provides durable,
...
tenant-scoped commands and acknowledgements through published `go-queue`
...
managed root `go-queue` as their native lifecycle controller and a native
...
status through the same `go-queue` management HTTP handler. Managed queues can
