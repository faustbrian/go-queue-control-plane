...
Build a production-grade open-source control plane for `go-queue` that provides
...
`go-queue` remains the data plane and owns delivery, worker semantics, native
backend adapters, retries, acknowledgement, pending recovery, and enforcement of
control commands. `go-queue-control-plane` owns fleet visibility, desired
...
- Depend only on stable management, event, worker-status, failure, dead-letter,
  and control contracts exported by `go-queue`.
...
- `go-queue` manages goroutine concurrency and graceful worker drain.
...
- Safe replay requiring explicit destination, confirmation, authorization, and
  idempotency policy.
- Queue purge/clear only where `go-queue` exposes safe backend semantics.
...
- Authentication through `go-authentication`.
- Authorization through `go-authorization` with explicit permissions for view,
...
- Historical metrics SHOULD be exported through `go-telemetry` to the platform
...
- PostgreSQL-first control, audit, and metadata storage through `go-postgres`
  with migrations through `go-migrations`.
- Valkey may carry ephemeral worker heartbeat and desired-state coordination
  through a native adapter where the `go-queue` protocol requires it.
...
- No backend-specific control operation bypassing `go-queue` contracts.
...
- `authz`: `go-authentication` and `go-authorization` integration.
...
- fake and real `go-queue` data-plane conformance suites
- Valkey 9 and Redis integration through `go-queue`, never duplicate clients
...
1. Freeze the `go-queue` management and protocol contracts required by control.
...
- No raw backend queue semantics are duplicated outside `go-queue`.
