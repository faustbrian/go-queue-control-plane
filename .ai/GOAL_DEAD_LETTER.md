...
`go-queue-control-plane` implementation. It complements `GOAL.md` and
...
letters through the stable contracts exported by `go-queue`.
...
`go-queue-control-plane` owns:

- authenticated and authorized administrative API, CLI, and optional UI;
- tenant-scoped orchestration of `go-queue` operations;
...
2. Identify which surfaces are contracts, mocks, or adapters and which are
   connected to a deployable `go-queue` implementation.
3. Preserve active implementation work and existing SemVer-governed behavior.
4. Reconcile every operation against `go-queue` capability negotiation and
...
- Use opaque bounded cursors supplied through `go-queue` contracts.
...
- Retry preserves the original logical destination and uses `go-queue` data-
  plane semantics.
...
- Use `go-authentication` for supported administrative authentication.
- Use `go-authorization` with default deny.
...
Expose through `go-telemetry` where honestly measurable:
...
- real end-to-end Valkey Streams and Redis Streams list, inspect, retry,
  replay, delete, purge, and capability tests through `go-queue`;
...
- contract tests against the versioned `go-queue` dead-letter surface.
...
- Backend operation and durability matrix sourced from `go-queue` evidence.
...
1. Reconcile the live control-plane implementation and `go-queue` capability
...
- Production deployments can list and inspect failures and dead letters through
  concrete tenant-scoped `go-queue` transports.
...
- No raw backend queue operation exists outside `go-queue`.
