...
- Pinned the completed `go-queue` dead-letter v1 contracts, stable operational
...
- Distinct durable `unsupported`, `timed_out`, and `partial` command outcomes
  so `go-queue` acknowledgements cannot be flattened into a clean failure.
...
- Tenant-scoped `go-queue` management adaptation for every control action,
...
- Optional production tenant command dispatch through the same authenticated
  `go-queue` management endpoints, with protocol-v1 translation, a bounded
...
- Pinned native Redis Streams failure and dead-letter reads plus retry,
  ...
  through authenticated `go-queue` contracts without native client access.
- Authenticated `go-queue` HTTP integration coverage for managed pause, resume,
...
- Pinned Redis 6.2.22 and Valkey 9.1.0 CI services proving real backend status,
  pause, and resume through `go-queue` and authenticated management HTTP.
- The real Valkey gate creates a failed delivery, dispatches replay, rejects a
  ...
  second public `go-queue` worker.
- Published `go-queue` Redis Streams and Valkey Streams status providers, with
  honest backend-version capability reporting, are pinned for the production
  worker and queue-status transport.
- Pinned `go-queue` retention capability negotiation, including exact-count
  reporting through Redis Streams and rolling-version HTTP integration.
- Authenticated tenant desired-state reads, a typed tenant-bound `go-queue`
...
- Native queue purge remains unavailable because `go-queue` intentionally
