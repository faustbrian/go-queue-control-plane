...
```json
{
  "idempotency_key": "maintenance-2026-07-16-1",
...
`go-queue` status source is wired. It requires `view` on `queue/queues`.
...
The production binary wires the authenticated `go-queue` HTTP status-reader
...
durable `go-queue` convergence record for one `queue`, `worker`, or
...
These routes exist when a tenant-scoped `go-queue` record source is wired:
...
bytes. Revealed data is base64-encoded JSON and remains bounded by `go-queue`
...
The production binary wires the authenticated `go-queue` record transport when
