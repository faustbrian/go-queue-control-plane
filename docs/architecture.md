...
`go-queue` is the data plane. It owns delivery, acknowledgement, retry,
...
`go-queue-control-plane` is the control plane. It owns authenticated
...
4. Dispatch through either the `go-queue` management adapter or the narrowly
...
write. That identifier is preserved through PostgreSQL, go-queue dispatch,
...
`failed` with the redacted code `dispatch_failed`. Published `go-queue`
...
binary and applied through `go-migrations`. Database access is provided through
`go-postgres` and pgx.
...
the stable `go-queue` record contract. A worker binds the typed client to its
...
`go-telemetry` runtime. Standard OpenTelemetry providers are passed directly to
...
- `alerts`: bounded platform-alert inputs from validated current snapshots and
  a fixed-label exporter using the deployment's `go-telemetry` meter.
...
- `dataplane`: tenant-scoped `go-queue` command and result adaptation.
