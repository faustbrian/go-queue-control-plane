...
process-manager architecture. Kubernetes supervises processes, `go-queue`
...
| Queue depth and wait | Honest backend measurements exported through `go-queue` and `go-telemetry` | Native current status is implemented; Redis 6.2 reports pending and oldest age but marks unavailable depth and lag unsupported, while Valkey 9 reports them. History export remains incomplete. |
...
| Retry one or many failures | Authorized, confirmed where bulk, idempotent, audited `go-queue` command | Redis Streams and Valkey Streams enforce retry and bounded bulk retry with backend-specific ambiguity guarantees. |
| Delete or purge failures | Confirmed, authorized, bounded `go-queue` command | Redis Streams and Valkey Streams enforce record delete and failure/dead-letter collection purge; queue purge remains unsupported. |
...
| Balancing | HPA/KEDA plus `go-queue` concurrency policy | Intentionally outside the control plane. |
...
`go-queue`; native queue purge reports unsupported until that package can
...
payload access. Assign each item either to Kubernetes/HPA/KEDA, `go-queue`, the
control plane, or the telemetry platform. Intentional gaps must have an
external owner before cutover.

During coexistence, keep Horizon and `go-queue` on separate queue ownership
