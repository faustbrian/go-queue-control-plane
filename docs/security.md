...
`go-authentication`. A request supplies a public key ID and secret in separate
...
Authorization uses `go-authorization` with deny-overrides ACL evaluation. Each
...
called. The stable `go-queue` contract caps revealed bytes at one mebibyte,
