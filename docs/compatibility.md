...
| Static API-key authentication and tenant ACL | available | Powered by `go-authentication` and `go-authorization`. |
| Durable command idempotency and outcomes | available | PostgreSQL-backed. |
| Desired pause/drain state | available | PostgreSQL-backed API and typed tenant reader feed the caller-supervised `go-queue` reconciler. |
| Tamper-evident audit history | available | Pagination, verification, and retention package APIs. |
| Worker fleet status | optional | Redis Streams and Valkey Streams workers expose native bounded status through authenticated `go-queue` HTTP when the management tenant document is configured. |
...
| Queue and worker lifecycle commands | available when managed | The configured tenant endpoint dispatches to a root `go-queue` with queue-owned pause, resume, drain, and terminate enforcement. |
...
| HTTP and command telemetry | optional | Explicit secure OTLP configuration through `go-telemetry`. |
| Queue status | optional | API, client, CLI, and authenticated `go-queue` HTTP transport are wired. Redis 6.2 explicitly marks unavailable depth and lag unsupported; Valkey 9 reports native depth, lag, and owned counters. |
...
| Queue history export | not wired | Historical metrics must use `go-telemetry`, not custom unbounded storage. |
...
The normal Go gate exercises the authenticated `go-queue` HTTP boundary with
...
duplicate command handling through a real managed `go-queue.Queue`. Real Redis
...
native `go-queue` adapters. Rolling-version
...
`go-queue` now provides stable versioned commands, terminal results, bounded
