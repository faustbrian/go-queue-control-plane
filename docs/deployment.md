...
| `QUEUE_CONTROL_TELEMETRY_ENABLED` | no | `false` | Enable owned `go-telemetry` OTLP export. |
...
The process owns a non-global `go-telemetry` runtime, supplies its standard
...
permission. Runtime retention changes remain unavailable until go-queue owns
