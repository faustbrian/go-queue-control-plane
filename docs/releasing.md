...
- fake and real `go-queue` management conformance;
- Redis and Valkey integration through `go-queue`, never direct clients;
...
Redis 6.2.22 and Valkey 9.1.0 lifecycle/status integration through `go-queue`,
...
```sh
VERSION=v1.2.3
cosign verify "ghcr.io/faustbrian/go-queue-control-plane:${VERSION}" \
  --certificate-identity "https://github.com/faustbrian/go-queue-control-plane/.github/workflows/release.yml@refs/tags/${VERSION}" \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
```

Replace the image name with `go-queue-control` to verify the CLI. Manual
