...
through versioned `go-queue/management` contracts and never acquires a Redis or
...
`go-queue`, Kubernetes/HPA/KEDA, or the telemetry platform. Process
...
unavailable; Redis and Valkey record operations stay inside `go-queue`;
...
| Backend | Real operations through `go-queue` HTTP and control dispatch |
| --- | --- |
...
remain explicitly unsupported because the pinned `go-queue` contract does not
...
plane features at the pinned `go-queue` revision:
...
Removing any blocker requires a published `go-queue` contract with equivalent
...
label `go-telemetry` bridge; before alerting is advertised, the deployment must
...
`go-queue`; meaningful coverage falls below 100%; or any required hosted gate
fails.
